...
object DataPreparation extends Logger {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val topicDescIndices = ldaModel.describeTopics(vocabSize).rdd.map {
      case Row(topic: Int, termIndices: Seq[Int], termWeights: Seq[Double]) => {
        termIndices.zip(termWeights).map(x => (topic, x._1, x._2))
      }
    }.flatMap(x => x).toDF("topic", "termIndex", "termWeight")
    ...
    val topicDesc = topicDescIndices.join(broadcast(vocabDf), Seq("termIndex"))
      .select("topic", "term", "termWeight")

    //For scalaLDAVis
    ldaModel.save("/tmp/scalaLDAvis/model/spark-lda")
    topicDesc.write.json("/tmp/scalaLDAvis/model/topic-dist")
    vocabDf.write.json("/tmp/scalaLDAvis/model/vocab")

    try {
      val (phi, thetaWithSize, termFreq, vocabRdd) =
        generateLDAVizData(sc, vocabSize, ldaModel, transformedDf, vocab)
  ...
  def generateLDAVizData(sc: SparkContext, vocabSize: Int,
